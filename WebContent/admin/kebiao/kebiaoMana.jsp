...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
        ...
        <script type="text/javascript">
        	//确认删除，传递到keibiao_servlet;携带type和id参数
           ...
           //课表添加，通过链接传递到keibiao.jsp,再有keibiao.jsp做表单处理
           function kebiaoAdd(){
			...
			function chaxunfangshi(){
				...
				if(y[x].value==1){
					...
					for(var i=document.getElementById("banji_id").options.length-1;i>=1;i--){
	                   ...
	                   document.getElementById("indicator").style.display="block";
	                   getAllService.teaAll(callbackTea);
				}else if(y[x].value==3){
					for(var i=document.getElementById("banji_id").options.length-1;i>=1;i--){
						document.getElementById("banji_id").options.remove(i);
					}
				           ...
				           <tr> 
				               <td>
				                  ...
				                  <select name="fangshi" id="fangshi" onchange="chaxunfangshi()">
						              ...
						              <option value="2" <c:if test="${requestScope.fangshi == 2}">selected</c:if>>教师</option>
						              <option value="3" <c:if test="${requestScope.fangshi == 3}">selected</c:if>>实验室</option>
						          </select>
				               </td>
				           </tr>
				    </table>
	...
	<script type='text/javascript'>
		if(${requestScope.fangshi == 1})
		{
            ...
            document.getElementById("indicator").style.display="block";
            getAllService.teaAll(callbackTea);
		}else if(${requestScope.fangshi == 3}){
			for(var i=document.getElementById("banji_id").options.length-1;i>=1;i--)
			{
				document.getElementById("banji_id").options.remove(i);
			}
